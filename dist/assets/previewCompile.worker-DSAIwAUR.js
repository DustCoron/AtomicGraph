...
`)}
    ...
    `,m=o?this.toReadableSource(d):this.toRawSource(d),E=this.hashString(m);return{backend:"glsl",source:m,warnings:Array.from(this.warnings),hash:E,atomsUsed:l,uniforms:Array.from(this.uniformDescriptors.values()),vertex:v,fragment:m,uniformBindings:this.uniformBindings}}glslOutputAssign(e){return e.type==="float"?`float v = ${e.code}; gl_FragColor = vec4(vec3(v), 1.0);`:e.type==="vec2"?`vec2 v = ${e.code}; gl_FragColor = vec4(v, 0.0, 1.0);`:e.type==="vec3"?`vec3 v = ${e.code}; gl_FragColor = vec4(v, 1.0);`:e.type==="vec4"?`vec4 v = ${e.code}; gl_FragColor = v;`:`gl_FragColor = ${e.code};`}assembleWgsl(e,o){const l=this.buildUniformLayout(),v=e.atoms??new Set,d=this.resolveAtomOrder(v),m=this.emitAtomPrelude(v),E=_e(this.wgslVariant),_=`
...
`,t=o?this.toReadableSource(_):this.toRawSource(_),s=this.hashString(t),W=this.rebuildGlslFromState(e,o);return{backend:"wgsl",source:W.source,warnings:Array.from(this.warnings),hash:s,atomsUsed:d,uniforms:Array.from(this.uniformDescriptors.values()),vertex:W.vertex,fragment:W.fragment,uniformBindings:this.uniformBindings,wgsl:t,uniformLayout:l}}rebuildGlslFromState(e,o){const l="varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}",v="precision highp float;varying vec2 vUv;void main(){gl_FragColor=vec4(0.,0.,0.,1.);}";return{source:v,vertex:l,fragment:v}}wgslOutputReturn(e){return e.type==="float"?`let v = ${e.code}; return vec4f(vec3f(v), 1.0);`:e.type==="vec2"?`let v = ${e.code}; return vec4f(v, 0.0, 1.0);`:e.type==="vec3"?`let v = ${e.code}; return vec4f(v, 1.0);`:e.type==="vec4"?`let v = ${e.code}; return v;`:`return ${e.code};`}wgslOutputColor(e){return e.type==="float"?`let v = ${e.code}; let col = vec4f(vec3f(v), 1.0);`:e.type==="vec2"?`let v = ${e.code}; let col = vec4f(v, 0.0, 1.0);`:e.type==="vec3"?`let v = ${e.code}; let col = vec4f(v, 1.0);`:e.type==="vec4"?`let v = ${e.code}; let col = v;`:`let col = ${e.code};`}buildUniformLayout(){const e=[];for(const[o,l]of this.uniformIndexMap.entries()){const v=this.uniformDescriptors.get(o),d=(v==null?void 0:v.type)||"float",m=d==="vec4"?4:d==="vec3"?3:d==="vec2"?2:1;e.push({name:o,type:d,index:l,size:m})}return e.sort((o,l)=>o.index-l.index)}sysIdx(e){return this.uniformIndexMap.get(e)??0}registerSystemUniforms(){const e=[["u_time","float",0],["u_resolution","vec2",[1,1]],["u_preview_offset","vec2",[0,0]],["u_preview_zoom","float",1],["u_preview_tile","float",0]];for(const[o,l,v]of e)this.uniformDescriptors.set(o,{name:o,type:l,nodeId:"system",key:o,defaultValue:v}),l==="vec2"?(this.bindings.setVec2(o,v),this.uniformIndexMap.set(o,this.nextUniformIndex),this.nextUniformIndex+=2):(this.bindings.setFloat(o,typeof v=="number"?v:0),this.uniformIndexMap.set(o,this.nextUniformIndex),this.nextUniformIndex+=1);this.uniformBindings=this.bindings.getUniforms()}defaultOutputExpression(e){const o=this.backend==="wgsl"?"vec3f":"vec3";return e==="baseColor"?{code:`${o}(0.0)`,type:"vec3"}:e==="normal"?{code:`${o}(0.5, 0.5, 1.0)`,type:"vec3"}:e==="ao"?{code:"1.0",type:"float"}:{code:"0.0",type:"float"}}getOutputExpression(e,o){const l=re[o]??0,v=this.defaultOutputExpression(o);return this.getSource(e,l,"uv",v.code,v.type)}defaultShader(){if(this.backend==="wgsl"){const o=ke;return{backend:"wgsl",source:"",warnings:[],hash:this.hashString(o),atomsUsed:[],uniforms:[],vertex:"",fragment:"",uniformBindings:{},wgsl:o,uniformLayout:[]}}const e="precision highp float;varying vec2 vUv;void main(){gl_FragColor=vec4(0.,0.,0.,1.);}";return{backend:"glsl",source:e,warnings:[],hash:this.hashString(e),atomsUsed:[],uniforms:[],vertex:"varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}",fragment:e,uniformBindings:{}}}addUniform(e,o,l){const v=me(e,o),d=this.inferUniformType(e,o,l);if(!this.uniformDescriptors.has(v)){Array.isArray(l)?l.length===2?this.bindings.setVec2(v,[l[0],l[1]]):l.length===3?this.bindings.setVec3(v,[l[0],l[1],l[2]]):l.length>=4?this.bindings.setVec4(v,[l[0],l[1],l[2],l[3]]):this.bindings.setFloat(v,0):typeof l=="boolean"?this.bindings.setBool(v,l):d==="int"?this.bindings.setInt(v,Number(l)):this.bindings.setFloat(v,Number(l)),this.uniformBindings=this.bindings.getUniforms(),this.uniformDescriptors.set(v,{name:v,type:d,nodeId:e,key:o,defaultValue:l});const m=d==="vec4"?4:d==="vec3"?3:d==="vec2"?2:1;this.uniformIndexMap.set(v,this.nextUniformIndex),this.nextUniformIndex+=m,this.backend==="glsl"&&this.decls.add(`uniform ${this.toGLSLType(d)} ${v};`)}if(this.backend==="wgsl"){const m=this.uniformIndexMap.get(v);return d==="vec2"?`uv2(${m}u)`:`uf(${m}u)`}return v}sysRef(e){return this.backend==="wgsl"?`uf(${this.uniformIndexMap.get(e)??0}u)`:e}sysRef2(e){return this.backend==="wgsl"?`uv2(${this.uniformIndexMap.get(e)??0}u)`:e}cloneExpr(e){return{code:e.code,type:e.type,atoms:e.atoms?new Set(e.atoms):void 0}}mergeAtoms(e,o){o&&o.forEach(l=>e.add(l))}mergeExprAtoms(e,o){o!=null&&o.atoms&&o.atoms.forEach(l=>e.add(l))}toWgslDataType(e){return e==="float"?"f32":e==="vec2"?"vec2f":e==="vec3"?"vec3f":e==="vec4"?"vec4f":"f32"}callAtom(e,o,l,v){const d=G[o],m=new Set;v.forEach(B=>this.mergeExprAtoms(m,B)),m.add(o);const E=`${o}(${v.map(B=>B.code).join(", ")})`;if(!d)return this.warnings.add(`Missing atom definition: ${o}`),{code:E,type:l,atoms:m};if(!this.W||!d.pure)return{code:E,type:l,atoms:m};const _=this.atomCse.get(e)||new Map;this.atomCse.set(e,_);const t=`${o}|${l}|${v.map(B=>B.code).join("|")}`,s=_.get(t);if(s){const B=new Set;return this.mergeExprAtoms(B,s),this.mergeAtoms(B,m),{code:s.code,type:s.type,atoms:B}}const W=this.tempCounterByFunc.get(e)??0,O=`t_${W}`;this.tempCounterByFunc.set(e,W+1);const r=this.funcBodies.get(e)||[];this.funcBodies.set(e,r),r.push(`let ${O}: ${this.toWgslDataType(l)} = ${E};`);const T={code:O,type:l,atoms:m};return _.set(t,T),T}resolveAtomOrder(e){if(e.size===0)return[];const o=new Set,l=[...e];for(;l.length;){const _=l.pop();if(o.has(_))continue;o.add(_);const t=G[_];if(!t){this.warnings.add(`Unknown atom referenced: ${_}`);continue}(t.deps??[]).forEach(s=>{o.has(s)||l.push(s)})}const v=new Set,d=new Set,m=[],E=_=>{if(v.has(_))return;if(d.has(_)){this.warnings.add(`Atom dependency cycle detected at "${_}"`);return}const t=G[_];if(!t){this.warnings.add(`Missing atom during prelude emit: ${_}`);return}d.add(_),[...t.deps??[]].sort().forEach(s=>{o.has(s)&&E(s)}),d.delete(_),v.add(_),m.push(_)};return[...o].sort().forEach(_=>E(_)),m}emitAtomPrelude(e){const o=this.resolveAtomOrder(e);return o.length===0?"":o.map(l=>G[l].wgsl.trim()).join(`

`)}getSource(e,o,l="uv",v="0.0",d="float",m){const E=this.edgeIndex.get(`${e}:${o}`);if(E){const _=this.genFunction(E.fromId,E.fromPort);return{code:`${_.code.split("(uv)").join(`(${l})`)}`,type:_.type,atoms:_.atoms?new Set(_.atoms):void 0}}return{code:v,type:d,atoms:m?new Set(m):void 0}}asFloat(e,o,l){return e.type==="float"?e.code:(this.warnings.add(`Implicit cast at node ${o}: ${e.type} -> float (${l})`),e.type==="vec2"||e.type==="vec3"||e.type==="vec4"?`(${e.code}).x`:`(${e.code})`)}asVec4(e){return e.type==="vec4"?e.code:e.type==="vec3"?this.v4(`${e.code}.x, ${e.code}.y, ${e.code}.z, 1.0`):e.type==="vec2"?this.v4(`${e.code}.x, ${e.code}.y, 0.0, 1.0`):this.v4(`${e.code}, ${e.code}, ${e.code}, 1.0`)}sanitizeAtomSubgraph(e){if(!e||!Array.isArray(e.nodes)||!Array.isArray(e.edges))return null;const o=e.nodes.filter(d=>d&&typeof d.id=="string"&&typeof d.type=="string"&&typeof d.x=="number"&&typeof d.y=="number").map(d=>({id:d.id,type:d.type,x:d.x,y:d.y,params:d.params&&typeof d.params=="object"?d.params:{}}));if(o.length===0)return null;const l=new Set(o.map(d=>d.id)),v=e.edges.filter(d=>d&&typeof d.id=="string"&&typeof d.fromId=="string"&&typeof d.toId=="string"&&typeof d.fromPort=="number"&&typeof d.toPort=="number"&&l.has(d.fromId)&&l.has(d.toId));return{schemaVersion:typeof e.schemaVersion=="number"?e.schemaVersion:1,nodes:o,edges:v,resolution:typeof e.resolution=="number"?e.resolution:void 0}}atomInputPortIndex(e){return e==="atom_input"||e==="atom_input_a"?0:e==="atom_input_b"?1:e==="atom_input_c"?2:e==="atom_input_d"?3:null}compileAtomSubgraph(e,o){var p;const l=o[0]?this.cloneExpr(o[0]):{code:"0.0",type:"float"},v=this.sanitizeAtomSubgraph((p=e.params)==null?void 0:p.subgraph);if(!v)return this.cloneExpr(l);const d=`atom_${e.id.replace(/[^a-zA-Z0-9_]/g,"_")}`,m=new Map,E=v.nodes.map(k=>{const N=`${d}_${k.id.replace(/[^a-zA-Z0-9_]/g,"_")}`;return m.set(k.id,N),{...k,id:N,params:{...k.params||{}}}}),_=v.edges.filter(k=>m.has(k.fromId)&&m.has(k.toId)).map(k=>({...k,id:`${d}_e_${k.id.replace(/[^a-zA-Z0-9_]/g,"_")}`,fromId:m.get(k.fromId),toId:m.get(k.toId)})),W=ie({nodes:E,edges:_}).height;if(!W)return this.warnings.add(`Atom graph ${e.id} has no connected Height output; using passthrough.`),this.cloneExpr(l);const O=new Map(E.map(k=>[k.id,k])),r=new Map;_.forEach(k=>{r.set(`${k.toId}:${k.toPort}`,k)});const T=this.nodes,B=this.edgeIndex;this.nodes=O,this.edgeIndex=r,this.atomInputExprStack.push(o.map(k=>this.cloneExpr(k)));try{return this.genFunction(W.fromId,W.fromPort)}catch(k){return this.warnings.add(`Atom graph ${e.id} compile failed: ${(k==null?void 0:k.message)||"unknown error"}`),this.cloneExpr(l)}finally{this.atomInputExprStack.pop(),this.nodes=T,this.edgeIndex=B}}get W(){return this.backend==="wgsl"}v2(e){return this.W?`vec2f(${e})`:`vec2(${e})`}v3(e){return this.W?`vec3f(${e})`:`vec3(${e})`}v4(e){return this.W?`vec4f(${e})`:`vec4(${e})`}modF(e,o){return this.W?`wmod(${e}, ${o})`:`mod(${e}, ${o})`}modV2(e,o){return this.W?`wmod2(${e}, ${o})`:`mod(${e}, ${o})`}atan2F(e,o){return this.W?`atan2(${e}, ${o})`:`atan(${e}, ${o})`}sel(e,o,l){return this.W?`select(${l}, ${o}, ${e})`:`((${e}) ? (${o}) : (${l}))`}tF(){return this.W?"f32":"float"}tI(){return this.W?"i32":"int"}tV2(){return this.W?"vec2f":"vec2"}genFunction(e,o=0){var N,ce,fe,pe;const l=this.nodes.get(e);if(!l)return{code:"0.0",type:"float"};const v=j[l.type],d=((N=v==null?void 0:v.outputs[o])==null?void 0:N.type)||"float",m=`${e}:${o}`,E=this.funcExprs.get(m);if(E)return this.cloneExpr(E);if(this.funcs.has(m))return{code:`${this.funcs.get(m)}(uv)`,type:d,atoms:void 0};const _=`fn_${e.replace(/[^a-zA-Z0-9]/g,"_")}_${o}`;if(l.type==="split"){const a=this.getSource(e,0,"uv",this.W?"vec4f(0.0)":"vec4(0.0)","vec4"),i=this.asVec4(a);if(o===4)return{code:`(${i}).xyz`,type:"vec3",atoms:a.atoms?new Set(a.atoms):void 0};const u=["x","y","z","w"][o]??"x";return{code:`(${i}).${u}`,type:"float",atoms:a.atoms?new Set(a.atoms):void 0}}this.funcs.set(m,_),this.funcBodies.set(m,[]),this.atomCse.set(m,new Map),this.tempCounterByFunc.set(m,0);const t=l.params,s=(a,i)=>this.addUniform(e,a,i),W=()=>this.sysRef("u_time"),O=new Set,r=(a,i="0.0",u="uv")=>{const c=typeof i=="string"?{code:i,type:"float"}:i,f=this.getSource(e,a,u,c.code,c.type,c.atoms);return this.mergeExprAtoms(O,f),f.type==="vec2"?(this.warnings.add(`Implicit cast at node ${l.type}: vec2 -> float (${e}, input ${a})`),`${f.code}.x`):f.type==="vec4"?(this.warnings.add(`Implicit cast at node ${l.type}: vec4 -> float (${e}, input ${a})`),`${f.code}.x`):f.code},T=(a,i="vec2(0.0)",u="uv")=>{const c=this.W?"vec2f(0.0)":i,f=this.getSource(e,a,u,c);return this.mergeExprAtoms(O,f),f.type==="float"?(this.warnings.add(`Implicit cast at node ${l.type}: float -> vec2 (${e}, input ${a})`),this.v2(f.code)):f.code},B=(a,i="vec3(0.0)",u="uv")=>{const c=this.W?"vec3f(0.0)":i,f=this.getSource(e,a,u,c,"vec3");return this.mergeExprAtoms(O,f),f.type==="float"?(this.warnings.add(`Implicit cast at node ${l.type}: float -> vec3 (${e}, input ${a})`),this.v3(`${f.code}, ${f.code}, ${f.code}`)):f.type==="vec2"?(this.warnings.add(`Implicit cast at node ${l.type}: vec2 -> vec3 (${e}, input ${a})`),this.v3(`${f.code}.x, ${f.code}.y, 0.0`)):f.type==="vec4"?(this.warnings.add(`Implicit cast at node ${l.type}: vec4 -> vec3 (${e}, input ${a})`),`(${f.code}).xyz`):f.code};let p="return 0.0;";switch(l.type){case"constant":p=`return ${s("val",t.value)};`;break;case"time":p=`return fract(${W()} * ${s("spd",t.speed)});`;break;case"uv_x":p="return uv.x;";break;case"uv_y":p="return uv.y;";break;case"uv":p="return uv;";break;case"uniform_color":{const a=s("r",t.r??.5),i=s("g",t.g??.5),u=s("b",t.b??.5);p=`return ${this.v3(`${a}, ${i}, ${u}`)};`;break}case"gaussian_noise":{const a=s("scale",t.scale??12),i=s("mean",t.mean??.5),u=s("stdDev",t.stdDev??.16),c=s("seed",t.seed??1337),f=s("tileOffsetX",t.tileOffsetX??0),y=s("tileOffsetY",t.tileOffsetY??0),x=s("nonSquare",t.nonSquare??!0?1:0),b=this.W?`(${c})`:`float(${c})`,w=this.sysRef2("u_resolution"),U=`${w}.x / max(${w}.y, 1.0)`,S=`(${this.sel(`${x} > 0.5`,this.v2(`uv.x * (${U}), uv.y`),"uv")} * max(${a}, 1.0) + ${this.v2(`${f}, ${y}`)})`,A=`max(hash2(floor(${S}) + ${this.v2(`${b} * 0.137, ${b} * 0.911`)}), 1e-5)`,q=`hash2(floor(${S}) + ${this.v2(`${b} * 0.271 + 19.0, ${b} * 0.613 + 73.0`)})`,L=`(sqrt(-2.0 * log(${A})) * cos(6.28318530718 * ${q}))`;p=`return clamp(${i} + ${L} * ${u}, 0.0, 1.0);`;break}case"tile_generator":{const a=s("scale",t.scale??6),i=s("radius",t.radius??.42),u=s("variation",t.variation??.25),c=s("seed",t.seed??1337),f=s("tileOffsetX",t.tileOffsetX??0),y=s("tileOffsetY",t.tileOffsetY??0),x=s("nonSquare",t.nonSquare??!0?1:0),b=this.W?`(${c})`:`float(${c})`,w=this.sysRef2("u_resolution"),U=`${w}.x / max(${w}.y, 1.0)`,S=`(${this.sel(`${x} > 0.5`,this.v2(`uv.x * (${U}), uv.y`),"uv")} * max(${a}, 1.0) + ${this.v2(`${f}, ${y}`)})`,A=`(fract(${S}) - 0.5)`,L=`hash2(${`floor(${S})`} + ${this.v2(`${b} * 0.151, ${b} * 0.337`)})`,F=`(1.0 - smoothstep(${i}, ${i} + 0.02, max(abs(${A}.x), abs(${A}.y))))`,P=`(1.0 - smoothstep(${i}, ${i} + 0.02, length(${A})))`;p=`return clamp(${t.shape==="dot"?P:F} * mix(1.0, ${L}, clamp(${u}, 0.0, 1.0)), 0.0, 1.0);`;break}case"noise":{const a=s("scale",t.scale??6),i=s("octaves",t.octaves??4),u=s("seed",t.seed??1337),c=s("tileOffsetX",t.tileOffsetX??t.seed??0),f=s("tileOffsetY",t.tileOffsetY??t.seed??0),y=s("nonSquare",t.nonSquare??!0?1:0),x=this.W?`(${u})`:`float(${u})`,b=this.sysRef2("u_resolution"),w=`${b}.x / max(${b}.y, 1.0)`;p=`return fbm(${`(${this.sel(`${y} > 0.5`,this.v2(`uv.x * (${w}), uv.y`),"uv")} * max(${a}, 0.00001) + ${this.v2(`${c}, ${f}`)} + ${this.v2(`${x} * 0.173, ${x} * 0.619`)})`}, ${i}, max(${a}, 0.00001), 1.0);`;break}case"perlin":{const a=s("scale",t.scale??32),i=s("disorder",t.disorder??0),u=s("disorderSpeed",t.disorderSpeed??1),c=s("seed",t.seed??1337),f=s("tileOffsetX",t.tileOffsetX??0),y=s("tileOffsetY",t.tileOffsetY??0),x=s("nonSquare",t.nonSquare??!0?1:0),b=this.sysRef2("u_resolution"),w=`${b}.x / max(${b}.y, 1.0)`,U=this.sel(`${x} > 0.5`,this.v2(`uv.x * (${w}), uv.y`),"uv"),I=`max(floor(${a} + 0.5), 1.0)`,S=`(${U} * ${I} + ${this.v2(`${f}, ${y}`)})`,A=this.W?`u32(max(${c}, 0.0))`:`float(${c})`,q=`(${this.sysRef("u_time")} * ${u})`;let L=S;if(this.W){const C=this.callAtom(m,"perlin2i_raw","float",[{code:`${S} + ${this.v2("17.0, 53.0")} + ${this.v2(`${q}*0.73, -${q}*0.41`)}`,type:"vec2"},{code:`(${A}) ^ 0x68bc21ebu`,type:"float"}]),Y=this.callAtom(m,"perlin2i_raw","float",[{code:`${S} + ${this.v2("113.0, 7.0")} + ${this.v2(`-${q}*0.29, ${q}*0.87`)}`,type:"vec2"},{code:`(${A}) ^ 0x02e5be93u`,type:"float"}]);this.mergeExprAtoms(O,C),this.mergeExprAtoms(O,Y),L=`(${S} + ${i} * ${this.v2(`${C.code}, ${Y.code}`)})`}else{const C=this.callAtom(m,"perlin2i_raw","float",[{code:`${S} + vec2(17.0,53.0) + vec2(${q}*0.73, -${q}*0.41)`,type:"vec2"},{code:`(${A}) + 101.0`,type:"float"}]),Y=this.callAtom(m,"perlin2i_raw","float",[{code:`${S} + vec2(113.0,7.0) + vec2(-${q}*0.29, ${q}*0.87)`,type:"vec2"},{code:`(${A}) + 211.0`,type:"float"}]);this.mergeExprAtoms(O,C),this.mergeExprAtoms(O,Y),L=`(${S} + ${i} * vec2(${C.code}, ${Y.code}))`}let F;this.W?(F=this.callAtom(m,"perlin2i_tiled","float",[{code:L,type:"vec2"},{code:I,type:"float"},{code:A,type:"float"}]),this.mergeExprAtoms(O,F)):F={code:`perlin2i_tiled(${L}, ${I}, ${A})`,type:"float"};const P=this.sel(`${a} < 1.5`,"0.5",F.code);if((ce=l.params)!=null&&ce.subgraph){const C=this.compileAtomSubgraph(l,[{code:P,type:"float"}]);this.mergeExprAtoms(O,C),p=`return ${this.asFloat(C,l.type,e)};`}else p=`return ${P};`;break}case"disorder":{const a=s("amount",t.amount??.08),i=s("scale",t.scale??8),u=s("speed",t.speed??.5),c=s("seed",t.seed??1337),f=this.W?`u32(max(${c}, 0.0))`:`float(${c})`,y=`(${this.sysRef("u_time")} * ${u})`,x=`(uv * max(${i}, 0.00001))`;let b,w;this.W?(b=this.callAtom(m,"perlin2i_raw","float",[{code:`${x} + vec2f(17.0,53.0) + vec2f(${y}*0.73, -${y}*0.41)`,type:"vec2"},{code:`(${f}) ^ 0x68bc21ebu`,type:"float"}]),w=this.callAtom(m,"perlin2i_raw","float",[{code:`${x} + vec2f(113.0,7.0) + vec2f(-${y}*0.29, ${y}*0.87)`,type:"vec2"},{code:`(${f}) ^ 0x02e5be93u`,type:"float"}])):(b=this.callAtom(m,"perlin2i_raw","float",[{code:`${x} + vec2(17.0,53.0) + vec2(${y}*0.73, -${y}*0.41)`,type:"vec2"},{code:`(${f}) + 101.0`,type:"float"}]),w=this.callAtom(m,"perlin2i_raw","float",[{code:`${x} + vec2(113.0,7.0) + vec2(-${y}*0.29, ${y}*0.87)`,type:"vec2"},{code:`(${f}) + 211.0`,type:"float"}])),this.mergeExprAtoms(O,b),this.mergeExprAtoms(O,w);const U=`(uv + ${a} * ${this.v2(`${b.code}, ${w.code}`)})`,I=this.callAtom(m,"perlin2i","float",[{code:x,type:"vec2"},{code:f,type:"float"}]);this.mergeExprAtoms(O,I),p=`return ${r(0,I,U)};`;break}case"voronoi":{const a=s("scale",t.scale??5),i=s("jitter",t.jitter??1),u=s("seed",t.seed??1337),c=s("tileOffsetX",t.tileOffsetX??t.seed??0),f=s("tileOffsetY",t.tileOffsetY??t.seed??0),y=s("nonSquare",t.nonSquare??!0?1:0),x=this.W?`(${u})`:`float(${u})`,b=this.sysRef2("u_resolution"),w=`${b}.x / max(${b}.y, 1.0)`,I=`(${this.sel(`${y} > 0.5`,this.v2(`uv.x * (${w}), uv.y`),"uv")} * max(${a}, 0.00001) + ${this.v2(`${c}, ${f}`)} + ${this.v2(`${x} * 0.137, ${x} * 0.911`)})`,S=`max(${a}, 0.00001)`,A=`voro(${I}, ${i}, ${S}, true)`,q=this.v4("cells,V.y,V.z,V.w");this.W?p=`{ var V=${A}; let f1=V.x; let cells=1.0-smoothstep(0.0,0.5,f1); return ${q}; }`:p=`{ vec4 V=${A}; float f1=V.x; float cells=1.0-smoothstep(0.0,0.5,f1); return ${q}; }`;break}case"worley":{const a=s("scale",t.scale??5),i=s("jitter",t.jitter??1),u=s("seed",t.seed??1337),c=s("tileOffsetX",t.tileOffsetX??t.seed??0),f=s("tileOffsetY",t.tileOffsetY??t.seed??0),y=s("nonSquare",t.nonSquare??!0?1:0),x=this.W?`(${u})`:`float(${u})`,b=this.sysRef2("u_resolution"),w=`${b}.x / max(${b}.y, 1.0)`;p=`return voro(${`(${this.sel(`${y} > 0.5`,this.v2(`uv.x * (${w}), uv.y`),"uv")} * max(${a}, 0.00001) + ${this.v2(`${c}, ${f}`)} + ${this.v2(`${x} * 0.137, ${x} * 0.911`)})`}, ${i}, max(${a}, 0.00001), true);`;break}case"bnw_spots2_v2":{const a=s("scale",t.scale??10),i=s("tileOffsetX",t.tileOffsetX??0),u=s("tileOffsetY",t.tileOffsetY??0),c=s("seed",t.seed??1337),f=s("nonSquareExpansion",t.nonSquareExpansion??!0?1:0),y=s("grainAmount",t.grainAmount??.22),x=s("grainThreshold",t.grainThreshold??.86),b=s("contrast",t.contrast??1.08),w=this.sysRef2("u_resolution"),U=this.v2(`${i}, ${u}`);if(this.W){const I=`u32(max(${c}, 0.0))`,S=this.callAtom(m,"bnw_spots2_v2","float",[{code:"uv",type:"vec2"},{code:`uv * ${w}`,type:"vec2"},{code:w,type:"vec2"},{code:a,type:"float"},{code:U,type:"vec2"},{code:I,type:"float"},{code:f,type:"float"},{code:y,type:"float"},{code:x,type:"float"},{code:b,type:"float"}]);this.mergeExprAtoms(O,S),p=`return ${S.code};`}else{const I=`float(${a})`,S=`float(${c})`,A=`clamp(floor(${I} + 0.5), 1.0, 32.0)`,q=`(${w}.x / max(${w}.y, 1.0))`,F=`(${this.sel(`${f} > 0.5`,`vec2(uv.x * (${q}), uv.y)`,"uv")} * ${A} + ${U})`,P=F,C=A,z=`clamp((${`smoothstep(0.35, 0.75, ${`(
          ...
          ) / 0.9375`})`} - 0.5) * max(${b}, 0.001) + 0.5, 0.0, 1.0)`,K=`perlin2i_tiled(${F} * 2.0 + vec2(13.2, -9.7), ${C} * 2.0, ${S} + 137.0)`,J=`clamp(${x} + (${K} - 0.5) * 0.12, 0.0, 1.0)`,ee=`hash2(floor(uv * ${w}) + vec2(${S} * 0.013, ${S} * 0.071))`,te=`step(${J}, ${ee})`;p=`return clamp(${z} - clamp(${y}, 0.0, 1.0) * ${te}, 0.0, 1.0);`}break}case"checker":p=`return ${this.modF(`floor(uv.x * ${s("sc",t.scale)}) + floor(uv.y * ${s("sc",t.scale)})`,"2.0")};`;break;case"panner":{const a=s("sx",t.speedX),i=s("sy",t.speedY),u=`uv + ${this.v2(`${a}, ${i}`)} * ${W()}`,c=`fbm(${u}, 4.0, 4.0, 0.0)`;p=`return ${r(0,c,u)};`;break}case"tile_sampler":{const a=s("scale",t.scale??6),i=`(${s("ang",(t.angle??0)*Math.PI/180)})`,u=s("tileOffsetX",t.tileOffsetX??0),c=s("tileOffsetY",t.tileOffsetY??0),f="(uv - 0.5)",x=`fract((${this.v2(`cos(${i}) * ${f}.x - sin(${i}) * ${f}.y, sin(${i}) * ${f}.x + cos(${i}) * ${f}.y`)} + 0.5) * max(${a}, 1.0) + ${this.v2(`${u}, ${c}`)})`;p=`return ${r(0,"0.0",x)};`;break}case"gradient":{const a=s("ang",t.angle*Math.PI/180);t.type==="radial"?p="return clamp(1.0 - length(uv - 0.5) * 2.0, 0.0, 1.0);":t.type==="angular"?this.W?p="{ var nx = uv - 0.5; return (atan2(nx.y, nx.x) / 3.14159 + 1.0) * 0.5; }":p="vec2 nx = uv - 0.5; return (atan(nx.y, nx.x) / 3.14159 + 1.0) * 0.5;":this.W?p=`{ var nx = uv - 0.5; return clamp(cos(${a}) * nx.x + sin(${a}) * nx.y + 0.5, 0.0, 1.0); }`:p=`vec2 nx = uv - 0.5; return clamp(cos(${a}) * nx.x + sin(${a}) * nx.y + 0.5, 0.0, 1.0);`;break}case"shape":{const a=t.type||"circle",i=this.v2(`${s("px",t.posX??.5)}, ${s("py",t.posY??.5)}`),u=s("sc",t.scale??.5),c=s("rad",t.rad??.5),f=s("bl",t.blur??.01),y=s("tk",t.thick??.05);let x;a==="ring"?x=`abs(sdCircle(sp, ${c})) - ${y}`:a==="square"?x=`sdBox(sp, ${this.v2(c)})`:a==="triangle"?x=`sdEquilateralTriangle(sp, ${c})`:a==="polygon"?x=`sdPolygon(sp, ${c}, 6)`:a==="star"?x=`sdStar(sp, ${c}, 5, 2.5)`:x=`sdCircle(sp, ${c})`,this.W?p=`{ var sp = (uv - ${i}) / max(${u}, 0.0001); let d = ${x}; return 1.0 - smoothstep(-${f}, ${f}, d); }`:p=`vec2 sp = (uv - ${i}) / max(${u}, 0.0001); float d = ${x}; return 1.0 - smoothstep(-${f}, ${f}, d);`;break}case"add":p=`return ${r(0)} + ${r(1,s("b",t.b))};`;break;case"subtract":p=`return ${r(0)} - ${r(1,s("b",t.b))};`;break;case"multiply":p=`return ${r(0,"1.0")} * ${r(1,s("b",t.b))};`;break;case"divide":p=`return ${r(0,"1.0")} / max(${r(1,s("b",t.b))}, 0.0001);`;break;case"power":p=`return pow(max(${r(0,"0.5")}, 0.0), ${r(1,s("exp",t.exp))});`;break;case"oneminus":p=`return 1.0 - ${r(0)};`;break;case"abs":p=`return abs(${r(0)});`;break;case"negate":p=`return -(${r(0)});`;break;case"sqrt":p=`return sqrt(max(${r(0)}, 0.0));`;break;case"sign":p=`return sign(${r(0)}) * 0.5 + 0.5;`;break;case"frac":p=`return fract(${r(0)});`;break;case"floor":p=`return floor(${r(0)});`;break;case"ceil":p=`return ceil(${r(0)});`;break;case"min2":p=`return min(${r(0)}, ${r(1,s("b",t.b))});`;break;case"max2":p=`return max(${r(0)}, ${r(1,s("b",t.b))});`;break;case"mod":p=`return ${this.modF(r(0),`max(${r(1,s("b",t.b))}, 0.001)`)};`;break;case"clamp01":p=`return clamp(${r(0)}, 0.0, 1.0);`;break;case"clamp":p=`return clamp(${r(0)}, ${s("lo",t.lo)}, ${s("hi",t.hi)});`;break;case"remap":{const a=s("il",t.inLo),i=s("ih",t.inHi),u=s("ol",t.outLo),c=s("oh",t.outHi);this.W?p=`{ let t = clamp((${r(0)} - ${a}) / max(${i} - ${a}, 0.001), 0.0, 1.0); return ${u} + t * (${c} - ${u}); }`:p=`float t = clamp((${r(0)} - ${a}) / max(${i} - ${a}, 0.001), 0.0, 1.0); return ${u} + t * (${c} - ${u});`;break}case"sin":p=`return sin(${r(0,"uv.x")} * ${s("freq",t.freq)} * 6.28318 + ${s("ph",t.phase)}) * 0.5 + 0.5;`;break;case"cos":p=`return cos(${r(0,"uv.x")} * ${s("freq",t.freq)} * 6.28318 + ${s("ph",t.phase)}) * 0.5 + 0.5;`;break;case"tan":p=`return clamp(tan(${r(0,"uv.x")} * ${s("freq",t.freq)} * 3.14159) * 0.08 + 0.5, 0.0, 1.0);`;break;case"atan2node":p=`return (${this.atan2F(r(0,"uv.y - 0.5"),r(1,"uv.x - 0.5"))} / 3.14159 + 1.0) * 0.5;`;break;case"lerp":p=`return mix(${r(0)}, ${r(1,"1.0")}, clamp(${r(2,s("t",t.t))}, 0.0, 1.0));`;break;case"smoothstep":p=`return smoothstep(${s("lo",t.lo)}, ${s("hi",t.hi)}, ${r(0,"uv.x")});`;break;case"step":p=`return step(${r(0,s("e",t.edge))}, ${r(1,"uv.x")});`;break;case"ifgt":p=`return ${this.sel(`(${r(0)}) > (${r(1,s("b",t.b))})`,r(2,"1.0"),r(3,"0.0"))};`;break;case"blend":{const a=r(0),i=r(1,"1.0"),u=s("op",t.opacity),c=t.mode;let f=i;c==="add"?f=`min(${a} + ${i}, 1.0)`:c==="multiply"?f=`(${a}) * (${i})`:c==="screen"?f=`1.0 - (1.0 - ${a}) * (1.0 - ${i})`:c==="overlay"?f=this.sel(`${a} < 0.5`,`2.0 * ${a} * ${i}`,`1.0 - 2.0 * (1.0 - ${a}) * (1.0 - ${i})`):c==="difference"?f=`abs(${a} - ${i})`:c==="dodge"?f=`clamp(${a} / (1.0 - ${i} + 0.001), 0.0, 1.0)`:c==="burn"&&(f=`clamp(1.0 - (1.0 - ${a}) / (${i} + 0.001), 0.0, 1.0)`),p=`return mix(${a}, ${f}, ${u});`;break}case"levels":{const a=s("il",t.inMin),i=s("ih",t.inMax),u=s("g",t.gamma);this.W?p=`{ let t = clamp((${r(0)} - ${a}) / max(${i} - ${a}, 0.001), 0.0, 1.0); return pow(t, 1.0 / max(${u}, 0.01)); }`:p=`float t = clamp((${r(0)} - ${a}) / max(${i} - ${a}, 0.001), 0.0, 1.0); return pow(t, 1.0 / max(${u}, 0.01));`;break}case"histogram_scan":{const a=s("pos",t.position??.5),i=s("wid",t.width??.1),u=s("ctr",t.contrast??1),c=r(0),f=`smoothstep(${a} - max(${i}, 0.0005), ${a}, ${c})`,y=`(1.0 - smoothstep(${a}, ${a} + max(${i}, 0.0005), ${c}))`;p=`return clamp(${f} * ${y} * ${u}, 0.0, 1.0);`;break}case"histogram_range":{const a=s("inMin",t.inMin??.15),i=s("inMax",t.inMax??.85),u=s("outMin",t.outMin??0),c=s("outMax",t.outMax??1),f=r(0);this.W?p=`{ let t = clamp((${f} - ${a}) / max(${i} - ${a}, 0.0001), 0.0, 1.0); return mix(${u}, ${c}, t); }`:p=`float t = clamp((${f} - ${a}) / max(${i} - ${a}, 0.0001), 0.0, 1.0); return mix(${u}, ${c}, t);`;break}case"curve":{const a=s("lift",t.lift??0),i=s("gamma",t.gamma??1),u=s("gain",t.gain??1);p=`return clamp(pow(clamp(${r(0)} + ${a}, 0.0, 1.0), 1.0 / max(${i}, 0.001)) * ${u}, 0.0, 1.0);`;break}case"transform_2d":{const a=s("offX",t.offsetX??0),i=s("offY",t.offsetY??0),u=`(${s("rot",t.rotation??0)} * 0.017453292519943295)`,c=s("sc",t.scale??1),f=s("tile",t.tile??!0?1:0),y=`((uv - 0.5) / max(${c}, 0.0001))`,b=`(${this.v2(`cos(${u}) * ${y}.x - sin(${u}) * ${y}.y, sin(${u}) * ${y}.x + cos(${u}) * ${y}.y`)} + 0.5 + ${this.v2(`${a}, ${i}`)})`,w=`clamp(${b}, ${this.v2("0.0, 0.0")}, ${this.v2("1.0, 1.0")})`,U=this.sel(`${f} > 0.5`,`fract(${b})`,w);p=`return ${r(0,"0.0",U)};`;break}case"safe_transform":{const a=s("offX",t.offsetX??0),i=s("offY",t.offsetY??0),u=s("sc",t.scale??1),c=s("tile",t.tile??!1?1:0),f=this.sysRef2("u_resolution"),y=`(0.5 / max(min(${f}.x, ${f}.y), 1.0))`,x=`((uv - 0.5) / max(${u}, 0.0001) + 0.5 + ${this.v2(`${a}, ${i}`)})`,b=`clamp(${x}, ${this.v2(`${y}, ${y}`)}, ${this.v2(`1.0 - ${y}, 1.0 - ${y}`)})`,w=this.sel(`${c} > 0.5`,`fract(${x})`,b);p=`return ${r(0,"0.0",w)};`;break}case"warp":{const a=T(1,this.W?"vec2f(0.0)":"vec2(0.0)"),i=s("str",t.strength),u=`(uv + ${a} * ${i})`;p=`return ${r(0,"0.0",u)};`;break}case"vector_warp":{const a=s("int",t.intensity??.15),i=s("ctr",t.centered??!0?1:0),u=s("tile",t.tile??!0?1:0),c=T(1,this.W?"vec2f(0.5)":"vec2(0.5)"),y=`(uv + (${this.sel(`${i} > 0.5`,`(${c} - ${this.v2("0.5, 0.5")})`,c)}) * ${a})`,x=this.sel(`${u} > 0.5`,`fract(${y})`,`clamp(${y}, ${this.v2("0.0, 0.0")}, ${this.v2("1.0, 1.0")})`);p=`return ${r(0,"0.0",x)};`;break}case"directional_warp":{const a=s("amount",t.amount??.15),i=s("angle",t.angle??0),u=r(1,"0.5"),f=`(uv + ${this.v2(`cos(${i}), sin(${i})`)} * ((${u} - 0.5) * ${a}))`;p=`return ${r(0,"0.0",f)};`;break}case"edge_detect":{const a=this.sysRef2("u_resolution"),i=`(${s("radius",t.radius)} / max(${a}.x, ${a}.y))`,u=s("strength",t.strength);this.W?p=`{
            ...
            return clamp(edge * ${u}, 0.0, 1.0);
          `;break}case"blur":{const a=s("amt",t.amount??.05),i=s("ctr",t.center??0),u=`${i} - ${a}`,c=`${i} + ${a}`;p=`return 1.0 - smoothstep(${u}, ${c}, ${r(0,"0.5")});`;break}case"highpass_grayscale":{const a=s("radius",t.radius??1),i=s("intensity",t.intensity??1),u=this.sysRef2("u_resolution"),c=`(max(${a}, 0.0001) / max(${u}.x, ${u}.y))`,f=r(0,"0.0"),y=r(0,"0.0",`(uv + ${this.v2(`${c}, 0.0`)})`),x=r(0,"0.0",`(uv - ${this.v2(`${c}, 0.0`)})`),b=r(0,"0.0",`(uv + ${this.v2(`0.0, ${c}`)})`),w=r(0,"0.0",`(uv - ${this.v2(`0.0, ${c}`)})`),U=`((${f} + ${y} + ${x} + ${b} + ${w}) * 0.2)`;p=`return clamp(((${f} - ${U}) * ${i}) + 0.5, 0.0, 1.0);`;break}case"slope_blur":{const a=s("intensity",t.intensity??2),i=s("samples",t.samples??4),u=this.sysRef2("u_resolution"),c=r(1,r(0,"0.5")),y=`(${`normalize(${this.v2(`${c} - 0.5, 1.0 - ${c}`)} + ${this.v2("1e-4, 1e-4")})`} * (${a} / max(${u}.x, ${u}.y)))`,x=`step(0.5, ${i})`,b=`step(1.5, ${i})`,w=`step(2.5, ${i})`,U=`step(3.5, ${i})`,I=r(0,"0.0"),S=r(0,"0.0",`(uv + ${y})`),A=r(0,"0.0",`(uv - ${y})`),q=r(0,"0.0",`(uv + ${y} * 2.0)`),L=r(0,"0.0",`(uv - ${y} * 2.0)`),F=r(0,"0.0",`(uv + ${y} * 3.0)`),P=r(0,"0.0",`(uv - ${y} * 3.0)`),C=r(0,"0.0",`(uv + ${y} * 4.0)`),Y=r(0,"0.0",`(uv - ${y} * 4.0)`),V=`(${I} + ${x} * (${S} + ${A}) + ${b} * (${q} + ${L}) + ${w} * (${F} + ${P}) + ${U} * (${C} + ${Y}))`,z=`(1.0 + 2.0 * (${x} + ${b} + ${w} + ${U}))`;p=`return clamp(${V} / max(${z}, 1.0), 0.0, 1.0);`;break}case"non_uniform_blur":{const a=s("radius",t.radius??2),i=s("samples",t.samples??4),u=this.sysRef2("u_resolution"),c=r(1,"1.0"),f=`(max(${a}, 0.0) * clamp(${c}, 0.0, 1.0) / max(${u}.x, ${u}.y))`,y=`step(0.5, ${i})`,x=`step(1.5, ${i})`,b=`step(2.5, ${i})`,w=`step(3.5, ${i})`,U=r(0,"0.0"),I=r(0,"0.0",`(uv + ${this.v2(`${f}, 0.0`)})`),S=r(0,"0.0",`(uv - ${this.v2(`${f}, 0.0`)})`),A=r(0,"0.0",`(uv + ${this.v2(`0.0, ${f}`)})`),q=r(0,"0.0",`(uv - ${this.v2(`0.0, ${f}`)})`),L=r(0,"0.0",`(uv + ${this.v2(`2.0 * ${f}, 0.0`)})`),F=r(0,"0.0",`(uv - ${this.v2(`2.0 * ${f}, 0.0`)})`),P=r(0,"0.0",`(uv + ${this.v2(`0.0, 2.0 * ${f}`)})`),C=r(0,"0.0",`(uv - ${this.v2(`0.0, 2.0 * ${f}`)})`),Y=r(0,"0.0",`(uv + ${this.v2(`3.0 * ${f}, 0.0`)})`),V=r(0,"0.0",`(uv - ${this.v2(`3.0 * ${f}, 0.0`)})`),z=r(0,"0.0",`(uv + ${this.v2(`0.0, 3.0 * ${f}`)})`),K=r(0,"0.0",`(uv - ${this.v2(`0.0, 3.0 * ${f}`)})`),J=r(0,"0.0",`(uv + ${this.v2(`4.0 * ${f}, 0.0`)})`),ee=r(0,"0.0",`(uv - ${this.v2(`4.0 * ${f}, 0.0`)})`),te=r(0,"0.0",`(uv + ${this.v2(`0.0, 4.0 * ${f}`)})`),Ue=r(0,"0.0",`(uv - ${this.v2(`0.0, 4.0 * ${f}`)})`),qe=`(${U}
          + ${y} * (${I} + ${S} + ${A} + ${q})
          + ${x} * (${L} + ${F} + ${P} + ${C})
          + ${b} * (${Y} + ${V} + ${z} + ${K})
          + ${w} * (${J} + ${ee} + ${te} + ${Ue}))`,Ie=`(1.0 + 4.0 * (${y} + ${x} + ${b} + ${w}))`;p=`return clamp(${qe} / max(${Ie}, 1.0), 0.0, 1.0);`;break}case"flood_fill":{const a=s("scale",t.scale??8),i=s("threshold",t.threshold??.1),u=s("seed",t.seed??1337),c=s("tileOffsetX",t.tileOffsetX??0),f=s("tileOffsetY",t.tileOffsetY??0),y=s("nonSquare",t.nonSquare??!0?1:0),x=this.W?`(${u})`:`float(${u})`,b=this.sysRef2("u_resolution"),w=`${b}.x / max(${b}.y, 1.0)`,A=`hash2(${`floor(${`(${this.sel(`${y} > 0.5`,this.v2(`uv.x * (${w}), uv.y`),"uv")} * max(${a}, 1.0) + ${this.v2(`${c}, ${f}`)})`})`} + ${this.v2(`${x} * 0.271, ${x} * 0.619`)})`,q=`step(${i}, ${r(0,"1.0")})`;p=`return ${A} * ${q};`;break}case"height_to_normal":{const a=this.sysRef2("u_resolution"),i=s("radius",t.radius??1),u=`(1.0 / max(${a}.x, ${a}.y))`,c=`(max(${i}, 0.0001) * ${u})`,f=s("strength",t.strength??1),y=s("flipY",t.flipY??!1?-1:1);this.W?p=`{
            ...
            return n * 0.5 + 0.5;
          }`;break}case"normal_combine":{const a=B(0,this.W?"vec3f(0.5, 0.5, 1.0)":"vec3(0.5, 0.5, 1.0)"),i=B(1,this.W?"vec3f(0.5, 0.5, 1.0)":"vec3(0.5, 0.5, 1.0)"),u=s("strength",t.strength??1);this.W?p=`{
            let b = normalize(${a} * 2.0 - 1.0);
            let d = normalize(${i} * 2.0 - 1.0);
            let ds = normalize(vec3f(d.xy * ${u}, d.z));
            let n = normalize(vec3f(b.xy + ds.xy, max(1e-4, b.z * ds.z)));
            return n * 0.5 + 0.5;
          }`:p=`{
            vec3 b = normalize(${a} * 2.0 - 1.0);
            vec3 d = normalize(${i} * 2.0 - 1.0);
            vec3 ds = normalize(vec3(d.xy * ${u}, d.z));
            vec3 n = normalize(vec3(b.xy + ds.xy, max(1e-4, b.z * ds.z)));
            return n * 0.5 + 0.5;
          }`;break}case"normal_normalize":{const a=B(0,this.W?"vec3f(0.5, 0.5, 1.0)":"vec3(0.5, 0.5, 1.0)"),i=s("flipY",t.flipY??!1?-1:1);this.W?p=`{
            let n0 = ${a} * 2.0 - 1.0;
            ...
            return n1 * 0.5 + 0.5;
          }`;break}case"remote":p="return 0.0;";break;case"atom_input":case"atom_input_a":case"atom_input_b":case"atom_input_c":case"atom_input_d":{const a=this.atomInputExprStack.length>0?this.atomInputExprStack[this.atomInputExprStack.length-1]:null,i=this.atomInputPortIndex(l.type)??0,u=(a==null?void 0:a[i])??(a==null?void 0:a[0])??null;if(!u){this.warnings.add(`Atom Input node "${e}" used outside atom graph.`),p="return 0.0;";break}a&&i>=a.length&&this.warnings.add(`Atom Input "${l.type}" requested missing parent port ${i}; using port 0.`),this.mergeExprAtoms(O,u),p=`return ${this.asFloat(u,l.type,`${e}`)};`;break}case"atom_graph":{const a=Math.max(1,((pe=(fe=j[l.type])==null?void 0:fe.inputs)==null?void 0:pe.length)||1),i=[];for(let c=0;c<a;c+=1){const f=this.getSource(e,c,"uv","0.0","float");i.push(f),this.mergeExprAtoms(O,f)}const u=this.compileAtomSubgraph(l,i);this.mergeExprAtoms(O,u),p=`return ${this.asFloat(u,l.type,e)};`;break}case"buffer":{if(o===1){const a=`1.0 / max(${this.sysRef2("u_resolution")}.x, 1.0)`;this.W?p=`{
              let s = ${a};
              let c  = ${r(0,"0.0","uv")};
              ...
              return (c + c1 + c2 + c3 + c4) * 0.2;
            `}else p=`return ${r(0)};`;break}case"output":p=`return ${r(0)};`;break;case"output_baseColor":p=`return ${r(0)};`;break;case"output_normal":p=`return ${r(0)};`;break;case"output_roughness":p=`return ${r(0)};`;break;case"output_height":p=`return ${r(0)};`;break;case"output_metallic":p=`return ${r(0)};`;break;case"output_ao":p=`return ${r(0)};`;break;default:p="return 0.5;";break}if(this.W){const a=this.toWgslDataType(d),u=[...this.funcBodies.get(m)||[],p].join(`
`),c=((v==null?void 0:v.label)||l.type).replace(/\r?\n/g," ");this.funcDefs.push(`// NodeRef id=${e} type=${l.type} port=${o} label=${c}
fn ${_}(uv: vec2f) -> ${a} {
${u}
}`)}else this.funcDefs.push(`// Node: ${(v==null?void 0:v.label)||l.type} (id=${e})
${d} ${_}(vec2 uv) { ${p} }`);const k={code:`${_}(uv)`,type:d,atoms:O.size>0?new Set(O):void 0};return this.funcExprs.set(m,k),this.cloneExpr(k)}inferUniformType(e,o,l){var m,E;if(Array.isArray(l))return l.length>=4?"vec4":l.length===3?"vec3":l.length===2?"vec2":"float";if(typeof l=="boolean")return"bool";const v=this.nodes.get(e),d=v?(E=(m=j[v.type])==null?void 0:m.params)==null?void 0:E[o]:void 0;return(d==null?void 0:d.type)==="int"?"int":"float"}toGLSLType(e){return e==="float"||e==="int"?"float":e==="bool"?"bool":e==="vec2"?"vec2":e==="vec3"?"vec3":"vec4"}toReadableSource(e){return e.split(`
...
`;function Ee($,e){return $==="bool"?!!e:$==="int"?Math.trunc(Number(e)||0):$==="float"?Number(e)||0:e}function Ae($,e,o){var m;if(e in o)return e;const l={val:"value",spd:"speed",sc:"scale",se:"seed",jt:"jitter",ang:"angle",px:"posX",py:"posY",r:"rot",rad:"rad",bl:"blur",tk:"thick",exp:"exp",freq:"freq",ph:"phase",t:"t",e:"edge",op:"opacity",g:"gamma",str:"strength",il:"inMin",ih:"inMax",ol:"outLo",oh:"outHi"},d=((m={add:{b:"b"},subtract:{b:"b"},multiply:{b:"b"},divide:{b:"b"},min2:{b:"b"},max2:{b:"b"},mod:{b:"b"},ifgt:{b:"b"},remap:{il:"inLo",ih:"inHi",ol:"outLo",oh:"outHi"},clamp:{lo:"lo",hi:"hi"},smoothstep:{lo:"lo",hi:"hi"},shape:{px:"posX",py:"posY",sc:"scale",bl:"blur",tk:"thick"},panner:{sx:"speedX",sy:"speedY"},levels:{il:"inMin",ih:"inMax",g:"gamma"},step:{e:"edge"}}[$])==null?void 0:m[e])||l[e];return d&&d in o?d:e}function Oe($,e){const o=new Map(e.nodes.map(v=>[v.id,v])),l={...$.uniformBindings};for(const v of $.uniforms){if(v.nodeId==="system")continue;const d=o.get(v.nodeId);if(!d)continue;const m=Ae(d.type,v.key,d.params);m in d.params&&(l[v.name]={value:Ee(v.type,d.params[m])})}return{...$,uniformBindings:l}}const le=self;let X=null,D="";const R=$=>{le.postMessage($)};le.onmessage=$=>{const e=$.data;if(!(!e||typeof e!="object"))try{if(e.type==="set_graph"){X=e.graph,D=e.signature;return}if(e.type!=="compile_job"||!X||e.signature!==D)return;const o=new we(X),l=e.mode==="output"?o.compile({backend:"glsl",readable:!1,outputChannel:e.outputChannel}):o.compile({backend:"glsl",readable:!1,nodeId:e.nodeId,nodeOutputPort:e.nodeOutputPort,outputChannel:e.outputChannel}),v=Oe(l,X);R({type:"compiled",signature:D,requestKey:e.requestKey,compiled:v})}catch(o){if(e.type==="compile_job"){R({type:"compile_error",signature:D||e.signature||"",requestKey:e.requestKey||"unknown",error:(o==null?void 0:o.message)||String(o)});return}R({type:"fatal",error:(o==null?void 0:o.message)||String(o)})}},R({type:"ready"})})();
